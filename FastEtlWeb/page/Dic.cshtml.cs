...
namespace FastEtlWeb.Pages
{
    public class DicModel : PageModel
    {
        ...
        public IActionResult OnPostDicList(int PageSize, int PageId,string DicId)
        {
            using (var db = new DataContext(AppEtl.Db))
            {
                ...
                if (string.IsNullOrEmpty(DicId))
                    list = FastRead.Query<Data_Dic_Details>(a => a.DicId != null).OrderBy<Data_Dic_Details>(a => new { a.DicId }).ToPage(page, db);
                else
                    list = FastRead.Query<Data_Dic_Details>(a => a.DicId == DicId).OrderBy<Data_Dic_Details>(a => new { a.DicId }).ToPage(page, db);
        ...
        public IActionResult OnPostDel(string id)
        {
            ...
            using (var db = new DataContext(AppEtl.Db))
            {
                if (FastRead.Query<Data_Business_Details>(a => a.Dic == id).ToCount(db) == 0)
                {
                    var dicId = FastRead.Query<Data_Dic_Details>(a => a.Id == id, a => new { a.DicId }).ToDic(db).GetValue("DicId").ToStr();
                    if (FastWrite.Delete<Data_Dic_Details>(a => a.Id == id, db).IsSuccess)
                    {
                        if (FastRead.Query<Data_Dic_Details>(a => a.DicId == dicId).ToCount(db) == 0)
                            FastWrite.Delete<Data_Dic>(a => a.Id == dicId);
